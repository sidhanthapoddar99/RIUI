...
void save(txtline *start)
{
    cout<<endl<<"enter the name of the file"<<endl;
    char savee[50];
    gets(savee);
    if(strcmp(savee,"data.txt")!=0){

    ofstream file(savee,ios::out|ios::app);
    while(start!=NULL)
    {
        file<<start->no<<"> "<<start->line<<"\n";
        start=start->next;
    }
    }
}
...
void function(txtline *start,txtline *end)
{
		...
		if (flag==1)
            ...
            cout<<endl<<"do you want to save this file:";
            char savee[50];
            gets(savee);
            if(strcmp(savee,"yes")==0)
                save(start);
}
